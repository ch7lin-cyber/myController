 ...
 *   0..80 %  : identified/interpolated range.
 *   80..100% : extrapolated equilibrium range using the 50..80 % slope.
 *              This region is simulation-only until additional measured
 *              identification data is available.
 *
 * With the current extrapolation and 25 degC ambient, the model predicts
 * approximately 168.63 degC equilibrium at 100 % MV. Therefore a controller
 * test with SV above that value cannot reach steady-state setpoint with this
 * model and should be interpreted as a plant-model range limitation.
 */
