...
/*
 * param         : public/reference gains, defined at
 *                 APP_FB_PID_REFERENCE_SAMPLE_TIME_MS.
 * runtime_param : gains actually used each cycle after Ts normalization.
 */
typedef struct
{
    APP_FB_PID_PARAMETER_T param;
    APP_FB_PID_PARAMETER_T runtime_param;
    APP_FB_PID_STATE_T state;
    uint32_t sample_time_ms;
    int32_t aw_max_correction_runtime;
    APP_FB_BOOL enable;
    APP_FB_BOOL integral_enable;
} APP_FB_PID_T;

/* Legacy/default init: assumes APP_FB_PID_REFERENCE_SAMPLE_TIME_MS. */
void app_fb_pid_init(
    APP_FB_PID_T *fb,
    const APP_FB_PID_PARAMETER_T *param);

/* Timing-aware init. sample_time_ms is fixed after initialization. */
