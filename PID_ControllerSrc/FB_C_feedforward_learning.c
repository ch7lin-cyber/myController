...
MY_API void app_fb_ff_learning_init(APP_FB_FF_LEARNING_T *fb, const APP_FB_ADAPTIVE_PARAMETER_T *param)
{
    ...
    if(param != 0)
    {
        if(param->error_threshold >= 0) fb->error_threshold = param->error_threshold;
        if(param->gain >= 0) fb->gain = param->gain;
        if(param->sv_change_threshold >= 0) fb->sv_change_threshold = param->sv_change_threshold;
        if(param->pid_deadband >= 0) fb->pid_deadband = param->pid_deadband;
        if(param->stable_count > 0) fb->stable_count = param->stable_count;
        if(param->freeze_count > 0) fb->freeze_count = param->freeze_count;
        if(param->offset_limit > 0) fb->offset_limit = param->offset_limit;
    }
    fb->counter = 0U;
    fb->offset = 0;
    fb->pid_sum = 0;
...
MY_API int32_t app_fb_ff_learning_run(APP_FB_FF_LEARNING_T *fb, int32_t sv, int32_t pv, int32_t pid_output, APP_FB_BOOL allow_learning)
{
    ...
    if(fb->learn_accumulator >= APP_FB_Q15_ONE || fb->learn_accumulator <= -APP_FB_Q15_ONE)
    {
        ...
        if((fb->offset >= fb->offset_limit && fb->learn_accumulator > 0) || (fb->offset <= -fb->offset_limit && fb->learn_accumulator < 0))
            app_fb_ff_learning_clear_fraction(fb);
    }
