...
int32_t app_fb_pid_run(APP_FB_PID_T *fb, APP_FB_TEMP sv, APP_FB_TEMP pv, int32_t d_filtered)
{
    ...
    if(fb->integral_enable == APP_FB_TRUE)
    {
        ...
        if(integral_candidate > (int64_t)fb->param.integral_limit) integral_candidate = (int64_t)fb->param.integral_limit;
        else if(integral_candidate < -(int64_t)fb->param.integral_limit) integral_candidate = -(int64_t)fb->param.integral_limit;
...
void app_fb_pid_bumpless_preload(APP_FB_PID_T *fb, APP_FB_TEMP sv, APP_FB_TEMP pv, int32_t d_filtered, int32_t desired_pid_output)
{
    ...
    integral_after_run = (desired_i_output * (int64_t)APP_FB_Q15_ONE) / (int64_t)fb->param.ki;
    integral_before_run = integral_after_run - (int64_t)error;
    if(integral_before_run > (int64_t)fb->param.integral_limit) integral_before_run = (int64_t)fb->param.integral_limit;
    else if(integral_before_run < -(int64_t)fb->param.integral_limit) integral_before_run = -(int64_t)fb->param.integral_limit;
...
void app_fb_pid_anti_windup(APP_FB_PID_T *fb, int32_t unsaturated_output, int32_t actual_output)
{
    ...
    if(limited_correction > (int64_t)APP_FB_PID_AW_MAX_CORRECTION) limited_correction = (int64_t)APP_FB_PID_AW_MAX_CORRECTION;
    else if(limited_correction < -(int64_t)APP_FB_PID_AW_MAX_CORRECTION) limited_correction = -(int64_t)APP_FB_PID_AW_MAX_CORRECTION;
    integral_candidate = (int64_t)fb->state.integral + limited_correction;
    if(integral_candidate > (int64_t)limit) integral_candidate = (int64_t)limit;
    else if(integral_candidate < -(int64_t)limit) integral_candidate = -(int64_t)limit;
    fb->state.integral = (int32_t)integral_candidate;
    if(fb->state.integral == limit || fb->state.integral == -limit) fb->state.aw_remainder = 0;
}
